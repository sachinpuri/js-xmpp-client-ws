...
var xmppClient={
    ...
    password:null,
    host:null,
    resource:'xmpp-javascript-library',    
    http_ws_url:null,
    onFailure:null,
    onSuccess:null,
    onDisconnect:null,
...
xmppClient.MUC = {
    ...
    joinRoom:function(roomJid, nickname, historyType, historyValue){
        var historyParams = {};
        switch(historyType){            
            case xmppClient.MUC.HistoryType.MAXCHARS:
                historyParams = {maxchars:historyValue};
                break;
            case xmppClient.MUC.HistoryType.MAXSTANZAS:
                historyParams = {maxstanzas:historyValue};
                break;
            case xmppClient.MUC.HistoryType.SECONDS:
                historyParams = {seconds:historyValue};
                break;
            case xmppClient.MUC.HistoryType.SINCE:
                historyParams = {since:historyValue};
                break;
            case xmppClient.MUC.HistoryType.NONE:
            default:
                historyParams = {maxchars:0};
                break;
        }
        ...
        xmlGenerator.start('history', historyParams, true).end().end();
        xmppClient.sendToServer(xmlGenerator.toString());
    },
...
xmppClient.parser={
    parseServerResponse:function(xml){
        ...
        //console.log("type: " + rootTagName);
        //console.log("type: " + $(xml).find(rootTagName.toLowerCase()).attr('type'));
        if(type === 'error'){
            this.parseError(xml);
        }else{
            switch(rootTagName){
